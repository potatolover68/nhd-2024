...
export function Carousel(props) {
  ...
  const carouselItemStyle = (index) => {
    ...
    const isPrev = (currentIndex === 0 && index === children.length - 1) ||
                  (index === currentIndex - 1);
    const isNext = (currentIndex === children.length - 1 && index === 0) ||
                  (index === currentIndex + 1);
  ...
  const indicatorStyle = (index) => ({
    ...
    '&:hover': {
      transform: 'scale(1.2)',
      background: index === currentIndex ? '#E18091' : 'rgba(255, 255, 255, 0.8)',
    },
  });
  ...
  return (
    ...
    >
      <button style={prevButtonStyle} onClick={prevSlide} aria-label="Previous slide">
        <img src="/assets/l-arrow.svg" alt="Previous" style={buttonIconStyle} />
      </button>
      {children.map((child, index) => (
        <div
          key={index}
          style={carouselItemStyle(index)}
        >
          {child}
        </div>
      ))}
      <button style={nextButtonStyle} onClick={nextSlide} aria-label="Next slide">
