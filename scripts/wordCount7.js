...
function extractContent(content) {
    ...
    // Remove text between quotes (single and double)
    content = content.replace(/'[^']*'/g, '');
    content = content.replace(/"[^"]*"/g, '');
    ...
    // Remove any remaining quoted content after line breaks
    const lines = content.split('\n');
    const cleanedLines = lines.map(line => {
        // If line starts with a quote-like pattern, remove it
        if (line.trim().match(/^["']|^Art(?:icle)?\s*\d+/)) {
            return '';
        }
        return line;
    });
...
function countWords(str) {
    // Split by whitespace and filter out empty strings and single-character items
    return str.split(/\s+/).filter(word => word.length > 1).length;
}
