...
function extractContent(content) {
    ...
    const cleanedLines = lines.map(line => {
        // Skip lines that look like quotes or articles
        if (line.match(/^["']|^Art(?:icle)?\s*\d+|^[A-Z][^.!?]*[.!?]\s*$|^The\s+[A-Z][^.!?]*[.!?]\s*$/)) {
            return '';
        }
...
function countWords(str) {
    // Split by whitespace and filter out empty strings, single characters, and common artifacts
    return str.split(/\s+/)
        .filter(word => word.length > 1 && 
            !word.match(/^[;(),.<>{}]/) && 
            !word.match(/^(const|function|return|export|default)$/))
        .length;
}
